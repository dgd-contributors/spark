...
private[spark] class DAGScheduler(
  ...
  private def checkBarrierStageWithDynamicAllocation(rdd: RDD[_]): Unit = {
    if (rdd.isBarrier() && Utils.isDynamicAllocationEnabled(sc.getConf)) {
      throw ExecutionErrors.barrierStageWithDynamicAllocationError()
    }
  }
    ...
    if (rdd.isBarrier()) {
      ...
      if (numPartitions > maxNumConcurrentTasks) {
        throw ExecutionErrors.
          numPartitionsGreateThanMaxNumConcurrentTasksError(numPartitions, maxNumConcurrentTasks)
      }
    }
  }
  ...
  def submitMapStage[K, V, C](
    ...
    if (rdd.partitions.length == 0) {
      throw ExecutionErrors.nonPartitionError()
    }
    ...
    event.accumUpdates.foreach { updates =>
      ...
      try {
        ...
        val acc: AccumulatorV2[Any, Any] = AccumulatorContext.get(id) match {
          ...
          case None =>
            throw ExecutionErrors.nonexistentAccumulator(id)
        }
  ...
  private def handleResubmittedFailure(task: Task[_], stage: Stage): Unit = {
    ...
    stage match {
      ...
      case _ =>
        throw ExecutionErrors.sendResubmittedTaskStatusForShuffleMapStagesOnlyError()
    }
  }
