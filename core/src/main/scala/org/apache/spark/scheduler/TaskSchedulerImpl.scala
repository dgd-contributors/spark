    ...
    try {
      ...
      case e: java.util.NoSuchElementException =>
        throw ExecutionErrors.
          unrecognizedSchedulerModeProperty(SCHEDULER_MODE_PROPERTY, schedulingModeConf)
    }
        ...
        if (launchedAnyTask && taskSet.isBarrier) {
          ...
          if (barrierPendingLaunchTasks.length != taskSet.numTasks) {
            if (legacyLocalityWaitReset) {
              ...
              throw ExecutionErrors.failResourceOffersForBarrierStage(errorMsg)
            } else {
          ...
          case Some(taskSet) =>
            if (state == TaskState.LOST) {
              ...
              val execId = taskIdToExecutorId.getOrElse(tid, {
                ...
                taskSet.abort(errorMsg)
                throw ExecutionErrors.markExecutorAsFailedError(errorMsg)
              })
  ...
  def error(message: String): Unit = {
    synchronized {
      if (taskSetsByStageIdAndAttempt.nonEmpty) {
        ...
        throw ExecutionErrors.clusterSchedulerError(message)
      }
    }
  }
