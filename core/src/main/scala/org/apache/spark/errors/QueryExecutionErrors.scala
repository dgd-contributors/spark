...
private[spark] object QueryExecutionErrors {
  ...
  def onlySupportFractionalResourceError(componentName: String): Throwable = {
  ...
  def notExistResourceScriptError(scriptFile: File, resourceName: String): Throwable = {
    new SparkException(s"Resource script: $scriptFile to discover $resourceName " +
      "doesn't exist!")
  }

  def notExistResourceError(resource: String, id: Int): Throwable = {
  ...
  def resourceProfileError(rpId: Int): Throwable = {
    new SparkException(s"ResourceProfileId $rpId not found!")
  }

  def conditionOfNumberOfCoresError(execCores: Int, taskCpus: Int ): Throwable = {
