  ...
  private def runMain(args: SparkSubmitArguments, uninitLog: Boolean): Unit = {
    ...
    try {
      ...
      case e: ClassNotFoundException =>
        ...
        throw ExecutionErrors.sparkUserAppError(CLASS_NOT_FOUND_EXIT_STATUS)
...
private[spark] object SparkSubmitUtils extends Logging {
  ...
  def loadIvySettings(
    ...
    try {
      ...
      case e @ (_: IOException | _: ParseException) =>
        throw ExecutionErrors.failedLoadIvySettingError(settingsFile, e)
    }
    processIvyPathArg(ivySettings, ivyPath)
    processRemoteRepoArg(ivySettings, remoteRepos)
    ivySettings
  ...
  def parseSparkConfProperty(pair: String): (String, String) = {
    pair.split("=", 2).toSeq match {
      case Seq(k, v) => (k, v)
      case _ => throw ExecutionErrors.lackSparkConfigError(pair)
    }
  }
  ...
  private[deploy] def getSubmitOperations(master: String): SparkSubmitOperation = {
    ...
    serviceLoaders.size match {
      case x if x > 1 =>
        throw ExecutionErrors.multipleExternalSparkSubmitOperationsRegisteredError(x, master)
