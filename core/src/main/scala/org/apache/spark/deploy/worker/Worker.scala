    ...
    case WorkDirCleanup =>
      ...
      try {
        val cleanupFuture: concurrent.Future[Unit] = concurrent.Future {
          ...
          if (appDirs == null) {
            throw ExecutionErrors.failedListFilesInAppDirsError(appDirs)
          }
          appDirs.filter { dir =>
      ...
      if (masterUrl != activeMasterUrl) {
        ...
        try {
          ...
          if (!executorDir.mkdirs()) {
            throw ExecutionErrors.failedCreateDirectoryError(executorDir)
          }
          ...
          val appLocalDirs = appDirectories.getOrElse(appId, {
            ...
            if (dirs.isEmpty) {
              throw ExecutionErrors.cannotCreateSubfolderInLocalRootDirsError(
