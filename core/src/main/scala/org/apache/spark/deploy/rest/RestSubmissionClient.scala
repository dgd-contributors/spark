      ...
      try {
        ...
        case e: SubmitRestConnectionException =>
          if (handleConnectionException(m)) {
            throw ExecutionErrors.unableConnectServerError(e)
          }
      }
    }
    response
  ...
  private def postJson(url: URL, json: String): SubmitRestProtocolResponse = {
    ...
    try {
      ...
      case e: ConnectException =>
        throw ExecutionErrors.connectServerError(e)
    }
    readResponse(conn)
  }
  ...
  private[rest] def readResponse(connection: HttpURLConnection): SubmitRestProtocolResponse = {
    ...
    val responseFuture = Future {
      ...
      if (responseCode != HttpServletResponse.SC_OK) {
        ...
        if (responseCode == HttpServletResponse.SC_INTERNAL_SERVER_ERROR &&
          ...
          throw ExecutionErrors.serverRespondedWithExceptionError(errString)
        }
        ...
        if (dataStream == null) {
          throw ExecutionErrors.serverReturnedEmptyBodyError()
        }
        ...
        response match {
          ...
          case unexpected =>
            throw ExecutionErrors.messageReceivedFromServerWasNotAResponseError(unexpected.toJson)
        }
      }
    }
    ...
    try { Await.result(responseFuture, 10.seconds) } catch {
      ...
      case unreachable @ (_: FileNotFoundException | _: SocketException) =>
        throw ExecutionErrors.unableConnectServerError(unreachable)
      case malformed @ (_: JsonProcessingException | _: SubmitRestProtocolException) =>
        throw ExecutionErrors.malformedResponseReceivedFromServerError(malformed)
      case timeout: TimeoutException => throw ExecutionErrors.noResponseFromServerError(timeout)
      case NonFatal(t) =>
        throw ExecutionErrors.waitingForResponseError(t)
    }
  }
