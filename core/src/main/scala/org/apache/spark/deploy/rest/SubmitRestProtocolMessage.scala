...
private[rest] abstract class SubmitRestProtocolMessage {
  ...
  final def validate(): Unit = {
    try {
      ...
      case e: Exception => throw ExecutionErrors.failedValidateMessageError(messageType, e)
    }
  }
  ...
  protected def doValidate(): Unit = {
    if (action == null) {
      throw ExecutionErrors.missActionFieldError(messageType)
    }
  }
  ...
  protected def assertFieldIsSet[T](value: T, name: String): Unit = {
    if (value == null) {
      throw ExecutionErrors.missFieldInMessageError(messageType, name)
    }
  }
...
private[spark] object SubmitRestProtocolMessage {
  ...
  def parseAction(json: String): String = {
    ...
    value.getOrElse {
      throw ExecutionErrors.notFoundActionFieldInJSONError(json)
    }
  }
