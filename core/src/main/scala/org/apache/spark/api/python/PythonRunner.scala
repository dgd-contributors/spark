    ...
    override def run(): Unit = Utils.logUncaughtExceptions {
      try {
        ...
        if (boundPort == -1) {
          ...
          logError(message)
          throw ExecutionErrors.serverSocketFailedBindJavaSideError(message)
        } else if (isBarrier) {
  ...
  abstract class ReaderIterator(
    ...
    protected val handleException: PartialFunction[Throwable, OUT] = {
      case e: Exception if context.isInterrupted =>
        ...
        throw ExecutionErrors.taskKilledUnknownReasonError(context)
      ...
      case eof: EOFException if faultHandlerEnabled && pid.isDefined &&
        ...
        throw ExecutionErrors.pythonWorkerExitedError(error, eof)

      case eof: EOFException =>
        throw ExecutionErrors.pythonWorkerExitedError(eof)
    }
  }
